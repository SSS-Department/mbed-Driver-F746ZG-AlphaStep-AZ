...
#define uint32SWAP(a) ((a << 16) & 0xFFFF0000) | ((a >> 16) & 0x0000FFFF)
...
struct TransactionSerial : SerialBase, NonCopyable<TransactionSerial<rxBufSize>> {
    ...
    inline TransactionSerial(
        ...
        PinName rxPin,
        int baud,
        std::chrono::microseconds frameDelim = 500us,   //487us,
    ...
    void transact(uint8_t* txBuf, int txLen, Callback<void(Result)> cb)
    {
        ...
        timeoutEvent = queue->call_in(rxTimeout, [this]()
        {
            timeoutHandler(); 
        });
    ...
    void transact(uint8_t* txBuf, int txLen)
    {
        rxIdx = 0;
        if (preTransmit) preTransmit();
        write(txBuf, txLen, NULL, SERIAL_EVENT_TX_COMPLETE);
    }
};
...
struct ModbusMaster : NonCopyable<ModbusMaster<txBufSize, rxBufSize>>{
    ...
    void transaction(FC fc, uint8_t slaveId, uint16_t addr, uint16_t num, uint8_t* val, CB cb)
    {
        txIdx = 0;
        complete = cb;
        ...
        // Serial Transaction
        stx.transact(adu, txIdx, [this, slaveId, fc](auto result)
        {
            ...
            if(static_cast<FC>(stx.rxBuf[1] & 0x7F) != fc) {
    ...
    void readCoils(uint8_t slaveId, uint16_t addr, uint16_t num, CB cb) {
        transaction(FC::readCoils, slaveId, addr, num, NULL, cb);
    }
    void readDiscreteInputs(uint8_t slaveId, uint16_t addr, uint16_t num, CB cb) {
        transaction(FC::readDiscreteInputs, slaveId, addr, num, NULL, cb);
    }
    void writeSingleCoil(uint8_t slaveId, uint16_t addr, bool val, CB cb) {
        uint16_t payload = __builtin_bswap16(val ? 0xFF00 : 0x0000);
        transaction(FC::writeSingleCoil, slaveId, addr, 1, reinterpret_cast<uint8_t*>(&payload), cb);
    }
    void writeMultipleCoils(uint8_t slaveId, uint16_t addr, uint16_t num, uint8_t* val, CB cb) {
        transaction(FC::writeMultipleCoils, slaveId, addr, num, val, cb);
    }    

    void readInputRegisters(uint8_t slaveId, uint16_t addr, uint16_t num, CB cb) {
        transaction(FC::readInputRegisters, slaveId, addr, num, NULL, cb);
    }
    void writeSingleRegister(uint8_t slaveId, uint16_t addr, uint16_t val, CB cb) {
        val = __builtin_bswap16(val);
        uint8_t* payload = reinterpret_cast<uint8_t*>(&val);
        transaction(FC::writeSingleRegister, slaveId, addr, 1, payload, cb);
    }

    void readHoldingRegisters(uint8_t slaveId, uint16_t addr, uint16_t num, CB cb)
    {
        transaction(FC::readHoldingRegisters, slaveId, addr, num, NULL, cb);
    }

    void writeMultipleRegisters(uint8_t slaveId, uint16_t addr, uint16_t num, uint16_t* val, CB cb)
    {
        for(int i = 0; i < num; i++) val[i] = __builtin_bswap16(val[i]);
        uint8_t* payload = reinterpret_cast<uint8_t*>(val);
        transaction(FC::writeMultipleRegisters, slaveId, addr, num, payload, cb);
    }
};
